...
void gpio_init(void)
{
	...
	PORTC->PCR[0] |= PORT_PCR_MUX(1); //switch
...
int main(void)
{
	...
	TPM1_C0V = (uint16_t)fit(0); //put on fire
	delay(500000);
	while(1){
		if (state==0){
			delay(1000000);
			...
			TPM1_C1V = (uint16_t)fit(92.1); //slow turn
			if (FPTC->PDIR & (1<<0)){
				c = 0.03;
			}
			else{
			...
			if (green>(double)red*(1.04 + c)){ //c = 1.04 for whites 
				delay(100);
